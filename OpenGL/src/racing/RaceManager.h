...
private:
	...
	// Последняя ОПУБЛИКОВАННАЯ карта кругов каждой машины. Снимок отдаёт её
	// разделяемым указателем, и пересобирается она, только если круги
	// изменились: иначе каждый кадр стоил бы копии std::map на машину.
	// mutable — публикация снимка логически константна.
	mutable std::map<int32_t, std::shared_ptr<const std::map<int, LapData>>> m_published_laps;
