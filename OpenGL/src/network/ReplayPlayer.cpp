...
namespace
{
    ...
    // Каждый шаг отката стоит доигрывания от снимка ПЛЮС полного пересчёта
    // гоночной логики, и всё это время поток отрисовки ждёт на мьютексе. При
    // 8 мс это выходило больше сотни пересчётов в секунду — картинка начинала
    // дёргаться от самой перемотки. Тридцать обновлений в секунду глаз
    // воспринимает как непрерывное движение, а нагрузка падает вчетверо.
    constexpr std::chrono::milliseconds SEEK_MIN_INTERVAL{ 33 };
    ...
    constexpr uint32_t KEYFRAME_INTERVAL_MS = 250;
