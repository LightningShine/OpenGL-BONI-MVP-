...
// Имя загруженной трассы (без пути и расширения) или пустая строка.
//
// Нужно записи телеметрии: в заголовке `.rjl` для этого есть поле, но до сих
// пор оно оставалось пустым, потому что имя нигде не хранилось. Из-за этого
// запись не сообщала, на какой трассе сделана, — а без трассы её нельзя ни
// воспроизвести, ни проверить, ту ли трассу открыл оператор. Для файла с карты
// памяти трекера это единственный способ понять, к чему он относится.
void set_loaded_track(const std::string& path);
std::string loaded_track_name();   // имя без пути и расширения
std::string loaded_track_path();   // полный путь к файлу трассы
