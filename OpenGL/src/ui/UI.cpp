...
#include <imgui.h>
#include <imgui_internal.h> // окна для пересчёта при смене DPI
...
void UI::LoadResources()
{
    int w, h;
    // ?????? ???? ? ???????????
    if (!LoadTextureFromFile("assets/images/start.png", &m_backgroundTexture, &w, &h))
    {
        std::cerr << "[UI] Warning: Background image not loaded\n";
    }
    
    // Load Icons
    if (!LoadTextureFromFile("assets/icons/PNG/file.png", &m_iconFile, nullptr, nullptr)) std::cerr << "Failed to load file.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/contact.png", &m_iconContact, nullptr, nullptr)) std::cerr << "Failed to load contact.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/copyright.png", &m_iconCopyright, nullptr, nullptr)) std::cerr << "Failed to load copyright.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/heart.png", &m_iconHeart, nullptr, nullptr)) std::cerr << "Failed to load heart.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/circle-x.png", &m_iconClose, nullptr, nullptr)) std::cerr << "Failed to load circle-x.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/DragAndDrop.png", &m_iconDragDrop, nullptr, nullptr)) std::cerr << "Failed to load DragAndDrop.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/Icon.png",       &m_logoTexture,   nullptr, nullptr)) std::cerr << "Failed to load Icon.png\n";

    // Транспорт повтора. Имена файлов — как их положил автор иконок.
    if (!LoadTextureFromFile("assets/icons/PNG/Play.png",       &m_iconPlay,        nullptr, nullptr)) std::cerr << "Failed to load Play.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/Paise.png",      &m_iconPause,       nullptr, nullptr)) std::cerr << "Failed to load Paise.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/LeftArrow.png",  &m_iconStepBack,    nullptr, nullptr)) std::cerr << "Failed to load LeftArrow.png\n";
    if (!LoadTextureFromFile("assets/icons/PNG/RighArrow.png",  &m_iconStepForward, nullptr, nullptr)) std::cerr << "Failed to load RighArrow.png\n";

    // Иконки-цифры 1..9 для боковых групп PRO-вида (512×512 PNG).
    for (int i = 0; i < 9; ++i) {
        char path[64];
        snprintf(path, sizeof(path), "assets/icons/PNG/%d PNG.png", i + 1);
        if (!LoadTextureFromFile(path, &m_numIcons[i], nullptr, nullptr))
            std::cerr << "Failed to load " << path << "\n";
    }
    
    // Load Compass texture
    if (!LoadTextureFromFile("assets/images/Compas scaled.png", &m_compassTexture, nullptr, nullptr)) 
    {
    ...
    LoadTextureFromFile("assets/icons/PNG/battery.png", &m_protoBatteryIconTexture, nullptr, nullptr);
    LoadTextureFromFile("assets/images/prototype.png", &m_protoPhotoTexture, nullptr, nullptr);
