...
namespace RaceConstants {
    ...
    // Круг до линии не считается вообще: таймер запускается В МОМЕНТ пересечения
    // (см. m_has_started_first_lap), поэтому первый же завершённый круг —
    // полноценный боевой, от линии до линии.
    static constexpr int LAP_START_NUMBER = 1;

    // Сколько завершённых кругов нужно, чтобы показывать лучшее время. Единица:
    // прогревочного круга в записях нет — до первого пересечения ничего не
    // фиксируется, отбрасывать нечего.
